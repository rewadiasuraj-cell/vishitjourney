...
targetFiles.forEach(file => {
  ...
  content = content.replace(/(<div class="footer-brand">\s*<div[^>]*?>\s*)<img[^>]+>/gi, (match, p1) => {
    return `${p1}<img src="Vishit_Journey_Logo.png" alt="Vishit Journey" style="height:60px;width:auto;display:block;">`;
  });

  // Backup fallback: if footer-brand has plain <img ...>
  content = content.replace(/(<div class="footer-brand">\s*)<img[^>]+>/gi, (match, p1) => {
    return `${p1}<div style="background:#fff;display:inline-block;padding:10px 16px;border-radius:8px;margin-bottom:.8rem;border:1px solid rgba(201,165,74,0.2)"><img src="Vishit_Journey_Logo.png" alt="Vishit Journey" style="height:60px;width:auto;display:block;"></div>`;
  });
