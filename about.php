...
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  /* ── HERO SECTION ── */
  .hero-section{
    position:relative;padding:9rem 2rem 5rem;
  ...
  .hero-title{font-family:'Cormorant Garamond',serif;font-size:3.5rem;color:var(--white);margin-bottom:1.5rem;font-weight:700;line-height:1.2}
  .hero-title span{color:var(--gold);font-style:italic}
  .hero-desc{max-width:750px;margin:0 auto 2.5rem;color:var(--muted);font-size:1.1rem}

  /* ── STATS SECTION ── */
  .stats-grid{
    display:grid;grid-template-columns:repeat(auto-fit, minmax(200px, 1fr));
    gap:1.5rem;max-width:1100px;margin:-3rem auto 4rem;padding:0 1.5rem;position:relative;z-index:10;
  }
  .stat-card{
    background:var(--card-bg);border:1px solid var(--border-gold);
    border-radius:16px;padding:2rem 1.5rem;text-align:center;backdrop-filter:blur(8px);
    transition:0.3s;
  }
  ...
  .stat-label{color:var(--offwhite);font-size:0.9rem;font-weight:500;text-transform:uppercase;letter-spacing:1px}
  ...
  .value-card{
    background:var(--card-bg);border:1px solid var(--border-gold);
    border-radius:16px;padding:2.2rem 1.8rem;transition:0.3s;
  }
...
<body>
  ...
  <div class="cta-banner">
    ...
    <a href="/contact.php" class="btn-contact" style="font-size:1.1rem;padding:0.8rem 2.2rem;">Book Free Consultation</a>
  </div>
